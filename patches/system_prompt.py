...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if agent._memory_store:
        # RAG memory: if vector index exists, memory is injected per-turn
        # (in conversation_loop) instead of here, keeping system prompt lean.
        _has_rag = (get_hermes_home() / "memory_index").exists()
        if agent._memory_enabled and not _has_rag:
